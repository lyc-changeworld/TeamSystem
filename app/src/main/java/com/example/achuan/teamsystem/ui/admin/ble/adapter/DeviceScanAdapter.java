package com.example.achuan.teamsystem.ui.admin.ble.adapter;
...
public class DeviceScanAdapter extends RecyclerView.Adapter<DeviceScanAdapter.ViewHolder> {
    ...
    protected List<BleDevice> mBleDeviceList;
    ...
    /*构造方法*/
    public DeviceScanAdapter(Context mContext, List<BleDevice> mBleDeviceList) {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int postion) {
        ...
        BleDevice bleDevice = mBleDeviceList.get(postion);


        final String deviceName = bleDevice.GetDevice().getName();
        if (deviceName != null && deviceName.length() > 0)
            holder.mTvName.setText(bleDevice.GetDevice().getName());
        else
            holder.mTvName.setText(R.string.unknown_device);

        holder.mTvAddress.setText("Mac: " + bleDevice.GetDevice().getAddress());

        double distance=bleDevice.getDistance();

        BigDecimal b   =   new   BigDecimal(distance);

        double   f1   =   b.setScale(2,   BigDecimal.ROUND_HALF_UP).doubleValue();

        holder.mTvRssi.setText("Rssi: "+bleDevice.GetAveragerssi()+" "+f1);
